...
public class RegistServlet extends HttpServlet {
	ServletContext context;// jsp에서의 application내장객체임
	int maxSize = 5 * 1024 * 1024;// 파일 최대 허용용량
	String savePath;// 임시 저장 경로(jvm메모리를 아끼기 위해서)
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 사용중인 운영환경에(win,linux,unix) 따른 상대 경로 구하기
		...
		try {
			...
			// 아이템을 구분하여 처리(텍스트는 dto에 담고 바이너리는 저장)
			Board board = new Board();
			for (FileItem item : itemList) {
				System.out.println("텍스트필드인지" + item.isFormField() + ",텍스트 필드는" + item.getFieldName() + "이고"
				...
				if (item.isFormField()) {
					...
					// 파일처리
					if (item.getSize() > 0) {// 파일의 바이트가 0이상일 경우= 업로드 하는경우
						long time = System.currentTimeMillis();
						String ext = FileManager.getExt(item.getName());
						File file = new File(savePath + time + "." + ext);
						try {
							// 임시 저장소에 생성된 임시파일이 제거되는 동시에 개발자가 지정한 파일명으로 새로운 파일 생성
							item.write(file);
							...
							out.print("<script>");
							if (result > 0) {
								out.print("alert('인서트 성공');");
								out.print("location.href='/board/list.jsp';");
							} else {
								out.print("alert('인서트 실패');");
